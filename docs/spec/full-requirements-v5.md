...
* `human_approval` gate ждёт approve, reject или rework
...
8. **Основной control-flow — DAG.**
   Назад можно идти только по явно заданным `rework_routes` и по явно разрешённому `need_more_input`.
...
nodes:
  ...
  - id: process-answers
    ...
    instruction: |
      ...
      If answers are insufficient, generate follow-up questions and return outcome need_more_input.
  ...
  - id: approve-code
    ...
    on_reject: fail
    on_rework_routes:
      discard_uncommitted: rollback-workspace
      keep_workspace: implement-feature
...
5. If provided answers are insufficient, generate new questions and return outcome need_more_input.
```
...
* `human_approval reject` moves to `on_reject`
...
обязан быть DAG, если не считать явно разрешённый route `need_more_input -> collect-answers`.

Backward edges разрешены только в:

* `on_rework_routes`
* явно разрешённом AI outcome `need_more_input`
...
* если node имеет `allowed_outcomes`, поле `outcome` обязательно
* `outcome` должен принадлежать `allowed_outcomes`
...
Approve, reject, rework и submit input выполняются только если:
...
* `POST /api/runs/{runId}/gates/{gateId}/reject`
...
Если ответов достаточно:

* outcome = `requirements_ready`
* выход = `requirements-draft`

Если ответов недостаточно:

* outcome = `need_more_input`
* выход = новый `questions`
* переход обратно в `collect-answers`
...
Если rework with `discard_uncommitted`:
...
* корректность `allowed_outcomes` и `outcome_routes`
...
15. Human approval gate поддерживает approve, reject и rework.
16. Rework идёт только по явно заданному route.
17. Flow поддерживает outcome based AI transitions, включая `need_more_input -> collect-answers`.
...
25. Human approval gate использует состояния `awaiting_decision`, `approved`, `rejected`, `rework_requested`.
