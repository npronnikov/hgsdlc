...
```
Что задаётся:
- instruction — команда (выполняется в zsh -lc)
...
| Роль | Права | Кто это |
|------|-------|---------|
| **OPERATOR** | Создаёт и запускает runs, работает с gates, просматривает артефакты | Разработчик, тимлид |
| **REVIEWER** | Только просмотр и работа с gates (approve/rework) | Ревьюер, аналитик |
| **ADMIN** | Всё + утверждение публикаций в каталог, управление настройками | Архитектор, тимлид |
