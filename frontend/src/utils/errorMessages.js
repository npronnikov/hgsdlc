...
const RULES = [
  ...
  { re: /Referenced rules mismatch coding_agent: (.+)/i, msg: (_, refs) => `Rule refs не соответствуют coding_agent: ${refs}` },
  { re: /Referenced skills mismatch coding_agent: (.+)/i, msg: (_, refs) => `Skill refs не соответствуют coding_agent: ${refs}` },
  ...
  { re: /Unable to parse flow_yaml: (.+)/i, msg: (_, detail) => `Не удалось разобрать flow_yaml: ${detail}` },
  {
    ...
    msg: (_, flowAgent, runtimeAgent) => `coding_agent flow (${flowAgent}) не совпадает с runtime settings (${runtimeAgent})`,
  },
  ...
  { re: /Start AI node requires instruction: (.+)/i, msg: (_, id) => `Стартовая AI-нода требует instruction: ${id}` },
