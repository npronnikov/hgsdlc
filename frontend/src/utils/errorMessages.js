...
const RULES = [
  ...
  { re: /execution_mode is required: (.+)/i, msg: (_, id) => `Нужен execution_mode: ${id}` },
  ...
  { re: /execution_mode must be agent for AI node: (.+)/i, msg: (_, id) => `execution_mode должен быть agent для AI-ноды: ${id}` },
  { re: /execution_mode must be command for command node: (.+)/i, msg: (_, id) => `execution_mode должен быть command для command-ноды: ${id}` },
  { re: /execution_mode must be human_input for human input node: (.+)/i, msg: (_, id) => `execution_mode должен быть human_input для human input-ноды: ${id}` },
  { re: /execution_mode must be human_approval for human approval node: (.+)/i, msg: (_, id) => `execution_mode должен быть human_approval для human approval-ноды: ${id}` },
  ...
  { re: /Executor node requires on_success or allowed_outcomes: (.+)/i, msg: (_, id) => `Executor-нода требует on_success или allowed_outcomes: ${id}` },
  {
    re: /Executor node requires outcome_routes when allowed_outcomes present: (.+)/i,
    msg: (_, id) => `Executor-нода требует outcome_routes при наличии allowed_outcomes: ${id}`,
  },
  { re: /Missing outcome route for (.+) in node: (.+)/i, msg: (_, outcome, id) => `Нет маршрута для исхода ${outcome} в ноде: ${id}` },
  {
    re: /Outcome route not declared in allowed_outcomes: (.+) -> (.+)/i,
    msg: (_, id, outcome) => `Маршрут ${outcome} не объявлен в allowed_outcomes: ${id}`,
  },
  { re: /Outcome routes require allowed_outcomes: (.+)/i, msg: (_, id) => `Outcome routes требуют allowed_outcomes: ${id}` },
  { re: /allowed_outcomes only supported for AI nodes: (.+)/i, msg: (_, id) => `allowed_outcomes поддерживается только для AI-ноды: ${id}` },
  ...
  { re: /human_approval gate requires on_reject: (.+)/i, msg: (_, id) => `human_approval gate требует on_reject: ${id}` },
