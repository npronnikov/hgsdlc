...
export function toNodeData(node, isStart) {
  ...
  return {
    ...
    commandEngine: node.command_engine || node.commandEngine || '',
    commandSpec: node.command_spec || node.commandSpec || null,
    successExitCodes: node.success_exit_codes || node.successExitCodes || [],
    maxFailureTransitions: node.max_failure_transitions ?? node.maxFailureTransitions ?? null,
